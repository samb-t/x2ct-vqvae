...
## Ref: https://github.com/vasily789/adaptive-weighted-gans
...
class aw_method():
    ...
    def aw_loss(self, Dloss_real, Dloss_fake, Dis_opt, Dis_Net, real_validity, fake_validity, scaler=None):
        Dis_opt.zero_grad()

        # computing real batch gradient
        scaler.scale(Dloss_real).backward(retain_graph=True) if scaler is not None else Dloss_real.backward(retain_graph=True)
        ...
        scaler.scale(Dloss_fake).backward() if scaler is not None else Dloss_fake.backward()#(retain_graph=True)
        # tensor with real gradients
