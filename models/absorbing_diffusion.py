...
class AbsorbingDiffusion(Sampler):
    def __init__(self, H, denoise_fn, mask_id, embedding_weight):
        ...
        self.shape = tuple(H.ct_config.model.latent_shape)
        self.num_timesteps = np.prod(self.shape)

        self.mask_id = mask_id
        self._denoise_fn = denoise_fn
        self.n_samples = H.diffusion.sampling_batch_size
        self.loss_type = H.diffusion.loss_type
        self.mask_schedule = H.diffusion.mask_schedule
        self.time_sampling = H.diffusion.time_sampling
    ...
    def _train_loss(self, x_0, context=None):
        ...
        # sample p(x_0 | x_t)
        x_0_hat_logits = self._denoise_fn(x_t, t=t, context=context).permute(0, 2, 1)
    ...
    def sample(self, context=None, temp=1.0, sample_steps=None):
        ...
        for t in reversed(sample_steps):
            ...
            x_0_logits = self._denoise_fn(x_t, t=t, context=context)
    ...
    def sample_mlm(self, context=None, temp=1.0, sample_steps=None):
        b, device = self.n_samples, 'cuda'
    ...
    def sample_max_probability(self, context=None, sample_steps=None):
        b, device = self.n_samples, 'cuda'
        x_t = torch.ones((b, np.prod(self.shape)), device=device).long() * self.mask_id
        unmasked = torch.zeros_like(x_t, device=device).bool()
        sample_steps = list(range(1, sample_steps+1))
        ...
        for t in reversed(sample_steps):
            ...
            x_0_logits = self._denoise_fn(x_t, t=t, context=context)
            x_0_hat = torch.max(x_0_logits, dim=-1)[1]
            x_t[changes] = x_0_hat[changes]
    ...
    def train_iter(self, x, context=None, test=False):
        loss, vb_loss = self._train_loss(x, context=context)
        if test:
            stats = {'test_loss': loss, 'test_vb_loss': vb_loss}
        else:
